        ...
        <div class="bg-white shadow-md rounded-lg p-6 mb-6">
            ...
            <div class="grid grid-cols-2 gap-4 items-center border-b pb-3 mb-3">
                <div>
                    ...
                    <span class="px-2 py-1 text-sm font-semibold rounded {{ $status_classes[$order->status] ?? 'bg-gray-200 text-gray-700' }}">
            ...
            <div class="flex justify-between items-center">
                @if ($order->status === 'pending' || $order->status === 'processing')
                    <form action="{{ route('order.cancel', $order->id) }}" method="POST" onsubmit="return confirm('Bạn có chắc muốn hủy đơn hàng này?');">
                        @csrf
                        @method('PUT') 
                        <button type="submit" class="bg-red-500 text-white px-3 py-1 rounded shadow sm:hover:bg-red-600 transition text-sm">
                            Hủy đơn
                        </button>
                    </form>
                ...
                @elseif ($order->status === 'delivered')
                    ...
                    <div class="text-red-500 font-semibold text-sm">
                        <form action="{{ route('order.delete', $order->id) }}" method="POST" onsubmit="return confirm('Bạn có chắc muốn xóa đơn hàng này?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="bg-red-500 text-white px-3 py-1 rounded shadow sm:hover:bg-red-600 transition text-sm">
                                Xóa đơn hàng
            ...
            @foreach ($order->details as $detail)
                <div class="flex items-center space-x-4 border-b py-4">
                    <img src="{{ asset('storage/' . $detail->product->image) }}" alt="{{ $detail->product->name }}" class="w-16 h-16 object-cover rounded shadow">
                    <div class="flex-1">
